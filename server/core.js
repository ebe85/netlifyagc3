      ...
      if(hrefAttr==null){
        ...
        if(hostnameHref==hostnameComing){
          ...
          if(isUrl(hrefAttr)==false){
            ...
            const pathHref = mapHref.pathname+mapHref.query;
            const dataReplace = option.origin+"/vlink-"+protoHref+hostnameHref+pathHref;
            $(el).attr(option.remake,dataReplace);
          };
        };
      };
...
<!-- XML Sitemap generated by NodeJs -->`);
      ...
      try{
        ...
        let dataOrigin="";
        if(urlPost.indexOf("vlink-")==0){
          let dataLink=await urlPost.split("vlink-")[1];
          linkPost=await dataLink.replace("https-","https://").replace("http-","http://");
          statusOrigin=false;
          dataOrigin=await "vlink-"+parseUrl(linkPost).origin.replace("https://","https-").replace("http://","http-");
        };
        if(isUrl(linkPost)){
